  ...
  .forEach((key) => {
    it(`screenshot testing ${key}`, async () => {
      ...
      await page.setContent(
        `<link href="http://localhost:3333/build/virtual-identity-challenge.css" rel="stylesheet" /><div class="st-test" style="display: inline-block; background-color: tomato;">${
          stories[key]()
        }</div>`,
      );
